...
def check_everything():
    """
    Hàm kiểm tra 2 điều kiện. 
    Trả về True nếu có thực hiện xử lý
    Trả về False nếu tất cả đều ổn.
    """
    global last_maintenance_time
    
    # 1. CHECK SPAWN(Y <= -11)
    pos = m.player_position()
    if pos and pos[1] <= Y_SPAWN:
        m.echo("§cPhát hiện ở spawn, đang /back...")
        m.chat("/home dung")
        time.sleep(5)
        return True 
    ...
    if time.time() - last_maintenance_time >= MAINTENANCE_INTERVAL:
        ...
        m.chat("/home dung")
        time.sleep(3)
        last_maintenance_time = time.time() # Reset đồng hồ
        return True 
